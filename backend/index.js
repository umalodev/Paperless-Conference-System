...
const validateMeetingStatus = async (meetingId) => {
  try {
    ...
    if (meeting.status !== 'active' && meeting.status !== 'started') {
      console.log(`Meeting ${meetingId} is not active/started (status: ${meeting.status})`);
      return false;
    }
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (message) => {
    try {
      ...
      if (data.participantId && !ws.userId) {
        ...
        console.log(`User ${data.participantId} identified in meeting ${meetingId}`);
      }
      ...
      validateMeetingStatus(meetingId).then(isValid => {
        ...
        wss.clients.forEach((client) => {
          console.log(`Client meetingId: ${client.meetingId}, readyState: ${client.readyState}, is same: ${client === ws}`);
          if (client !== ws && 
        ...
        console.log(`Message broadcasted to ${clientCount} clients in meeting ${meetingId}`);
        
        // Special handling for participant_ready_to_receive message
        if (data.type === 'participant_ready_to_receive') {
          console.log(`Participant ${data.from} is ready to receive video in meeting ${meetingId}`);
          ...
          wss.clients.forEach((client) => {
            if (client !== ws && 
              ...
              console.log(`Forwarding participant_ready_to_receive to client ${client.userId || 'unknown'}`);
        ...
        if (data.type === 'participant_joined') {
          console.log(`Participant ${data.participantId} joined meeting ${meetingId}`);
  ...
  ws.on('close', (event) => {
    console.log(`WebSocket disconnected from meeting: ${meetingId}, user: ${ws.userId}, code: ${event.code}, reason: ${event.reason}`);
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(`WebSocket server available at ws://localhost:${PORT}/meeting/{meetingId}`);
    });
  } catch (error) {
