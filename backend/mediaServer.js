...
io.on('connection', (socket) => {
  ...
  socket.on('connect-transport', async (data) => {
    try {
      ...
      const room = Array.from(rooms.values()).find(r => r.transports.has(transportId));
  ...
  socket.on('resume-consumer', async (data) => {
    try {
      ...
      const room = Array.from(rooms.values()).find(r => r.consumers.has(consumerId));
  ...
  socket.on('disconnect', () => {
    ...
    for (const [roomId, room] of rooms) {
      ...
      if (peer) {
        ...
        // Notify other peers
        socket.to(roomId).emit('peer-left', { peerId: peer.id, socketId: socket.id });
