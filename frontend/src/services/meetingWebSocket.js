...
class MeetingWebSocketService {
  ...
  async connect(meetingId, userId, apiUrl) {
    ...
    try {
      ...
      const token = localStorage.getItem("token"); // ambil JWT yang sama dipakai REST API
      const sep = apiUrl.includes("?") ? "&" : "?";
      const wsUrl = `${apiUrl.replace(
        /^http/,
        "ws"
      )}/meeting/${meetingId}${sep}token=${encodeURIComponent(token || "")}`;
      ...
      this.ws.onclose = (event) => {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
          ...
          console.log(
            `Meeting WebSocket reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`
          );
  ...
  send(message) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      console.warn(
        "Meeting WebSocket not connected, cannot send message:",
        message
      );
    }
  }
